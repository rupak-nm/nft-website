...
const Filter = ({ filters = [], properties, setProperties, showFilter, onFilterClose }) => {
  ...
  const [levelAndFamily, setLevelAndFamily] = useState({ level: null, family: null, lastChanged: '' })
  ...
  const handleClearAll = () => {
    setLevelAndFamily({ family: null, level: null, lastChanged: '' })
  ...
  const handleClearFilter = (key) => {
    if (key === 'Level') { setLevelAndFamily(prev => { return { ...prev, level: '', lastChanged: 'Family' } }) }
    if (key === 'Family') { setLevelAndFamily(prev => { return { ...prev, family: '', lastChanged: 'Level' } }) }
                    ...
                    <ul>
                      {
                        filter.values.map((value, idx) => {
                          const checked = selectedFilters.find(f => {
                            return f.key === filter.key && f.value.toString() === value.toString()
                          })
                          return (
                            <li className='option' key={idx}>
                              <Radio
                                ...
                                disabled={filter.enabled ? !filter.enabled.includes(value) : false}
